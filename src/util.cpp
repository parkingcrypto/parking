...
const char * const BITCOIN_CONF_FILENAME = "parking.conf";
const char * const BITCOIN_PID_FILENAME = "parkingd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "parking" is a composite category enabling all Parking-related debug output
                if(ptrCategory->count(std::string("parking"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ParkingCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ParkingCore
    // Mac: ~/Library/Application Support/ParkingCore
    // Unix: ~/.parkingcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ParkingCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ParkingCore";
#else
    // Unix
    return pathRet / ".parkingcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty parking.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override parking.conf
