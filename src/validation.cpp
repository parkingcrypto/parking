...
#if defined(NDEBUG)
# error "Parking Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Parking Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        nSubsidy = 600000 * COIN; // premine
    }

    if(nPrevHeight > nMNPIBlock)                  nSubsidy += 0.5 * COIN; // 1.5 PARK
    if(nPrevHeight > nMNPIBlock+(nMNPIPeriod* 1)) nSubsidy += 0.5 * COIN; // 2.0 PARK
    if(nPrevHeight > nMNPIBlock+(nMNPIPeriod* 2)) nSubsidy += 0.5 * COIN; // 2.5 PARK
    if(nPrevHeight > nMNPIBlock+(nMNPIPeriod* 3)) nSubsidy += 0.5 * COIN; // 3.0 PARK

    // yearly decline of production by 10% per year, projected ~15M coins max by year 2050+.
...
{
    CAmount ret = blockValue * 0.9; // start at 90%

    int nMNPIBlock = Params().GetConsensus().nMasternodePaymentsIncreaseBlock;
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;

                                                                          // mainnet:
    if(nHeight > nMNPIBlock)                   ret -= blockValue * 0.025; // 87.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  1)) ret -= blockValue * 0.025; // 85.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  2)) ret -= blockValue * 0.025; // 82.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  3)) ret -= blockValue * 0.025; // 80.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  4)) ret -= blockValue * 0.025; // 77.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  5)) ret -= blockValue * 0.025; // 75.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  6)) ret -= blockValue * 0.025; // 72.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  7)) ret -= blockValue * 0.025; // 70.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  8)) ret -= blockValue * 0.025; // 67.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod*  9)) ret -= blockValue * 0.025; // 65.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 10)) ret -= blockValue * 0.025; // 62.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 11)) ret -= blockValue * 0.025; // 60.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 12)) ret -= blockValue * 0.025; // 57.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 13)) ret -= blockValue * 0.025; // 55.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 14)) ret -= blockValue * 0.025; // 52.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 15)) ret -= blockValue * 0.025; // 50.0%
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // PARK : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(PARK): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(PARK): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PARK : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(PARK): spork is off, skipping transaction locking checks\n");
    }
