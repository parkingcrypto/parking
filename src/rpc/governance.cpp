...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in parking.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("parking.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Parking features */
    { "parking",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "parking",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "parking",               "gobject",                &gobject,                true,  {} },
    { "parking",               "voteraw",                &voteraw,                true,  {} },
